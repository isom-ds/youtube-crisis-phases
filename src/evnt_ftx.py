...
enddate = datetime.strptime('2023-01-15', '%Y-%m-%d')
...
for i, j in zip (dates, dates1):
    ...
    try:
        vid, id = youtubeSearch(query='ftx', key=key, start=i, end=j)
        dd = i
    except noVideos:
        print("No Videos")
...
with open("data/ftx_videos.json", "w") as outfile:
...
# %%
with open("data/ftx_stats.json", "w") as outfile:
...
for i, j in zip(range(0, len(idx), 100), list(range(99, len(idx), 100))+[(len(idx))]):
    ...
    with open(f"data/comments/ftx_comments_{j}.json", "w") as outfile:
...
counter = 1
for i in [x for x in os.listdir('data/comments') if x[:3]=='ftx']:
    # Read file
    print(f"{counter} - {i}")
    ...
    # Save
    with open(f"data/comments/processed/ftx_comments_{counter}.json", "w") as outfile:
        json.dump(x, outfile)
...
files = [x for x in os.listdir('data/comments/processed/') if x[:3]=='ftx']
for i in files:
    # Read file
    print(i)
